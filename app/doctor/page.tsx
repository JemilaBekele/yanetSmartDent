          ...
          <TableBody>
            {patients.map(({ _id, firstname, sex, orderId, finish }, index) => (
              <TableRow key={_id}>
                ...
                <TableCell>
  <span className={finish ? 'text-green-500' : 'text-red-500'}>
    {finish ? 'Finished' : 'Not Finished'}
  </span>
</TableCell>
      ...
      {selectedOrderId && (
        <OrderUpdateModal isOpen={isModalOpen} onClose={closeModal} orderId={selectedOrderId} />
      )}
    </div>
  );
};
