...
const SVGPainter: React.FC<SVGPainterProps> = ({
  ...
  // UPDATED: Render tooth structure based on anatomical root table
  const renderToothStructure = () => {
    // Upper incisors & canines (6-11) - Single root
    const isUpperAnterior = [4,6, 7, 8, 9, 10, 11, 13].includes(toothNumber);
    // Upper premolars (4-5, 12-13) - 2 roots (Buccal, Palatal)
    const isUpperPremolar = [5, 12].includes(toothNumber);
    ...
    // Single root teeth (Upper anterior, Lower anterior, Lower premolars)
    if (isUpperAnterior || isLowerAnterior || isLowerPremolar) {
      return (
        <>
          {/* Crown */}
          <path
    ...
    // Upper premolars - 2 roots (Buccal, Palatal)
      ...
      return (
        <>
          ...
          {/* Buccal Root (left) */}
          <path
            d="M 70 120 Q 80 200 90 120 L 90 280 Q 80 290 70 280 Z"
            fill="#f8f8f8"
            stroke="#333"
            strokeWidth="2"
          />
          {/* Palatal Root (right) */}
          <path
            d="M 110 120 Q 120 200 130 120 L 130 280 Q 120 290 110 280 Z"
            fill="#f8f8f8"
            stroke="#333"
            strokeWidth="2"
          />
    ...
    // Upper molars - 3 roots (Mesiobuccal, Distobuccal, Palatal)
  ...
  const getRootDescription = () => {
    if ([4,6, 7, 8, 9, 10, 11,13].includes(toothNumber)) return "Single Root (Upper Anterior)";
    if ([5, 12,].includes(toothNumber)) return "2 Roots - Buccal, Palatal (Upper Premolar)";
    if ([1, 2, 3, 14, 15, 16].includes(toothNumber)) return "3 Roots - MB, DB, Palatal (Upper Molar)";
    if ([22, 23, 24, 25, 26, 27].includes(toothNumber)) return "Single Root (Lower Anterior)";
    if ([20, 21, 28, 29].includes(toothNumber)) return "Single Root (Lower Premolar)";
    if ([17, 18, 19, 30, 31, 32].includes(toothNumber)) return "2 Roots - Mesial, Distal (Lower Molar)";
    ...
    <div className="bg-white p-4 rounded-lg border border-gray-300">
      <div className="flex justify-between items-center mb-4">
        <div>
          <h4 className="font-semibold text-gray-700">
            Freehand Painter - Tooth {toothNumber}
          </h4>
      ...
      <div className="border-2 border-gray-300 rounded-lg bg-gradient-to-br from-gray-50 to-gray-100 p-2 mb-3">
        ...
        >
          ...
          {/* Base tooth structure - UPDATED based on anatomical table */}
