...
const ToothImage = ({ toothNumber }: { toothNumber: number }) => {
  return (
    <div className="flex flex-col items-center">
      ...
      <span className="text-xs font-semibold text-gray-600 mt-1"></span>
    </div>
  );
};
...
// Main Edit Component
export default function ChildDentalChartEdit({ params }: DentalChartEditProps) {
  ...
  const handleSave = async () => {
    try {
      setFormMessage("Updating child dental chart...");
      setFormType(null);
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to update child dental chart");
      }
      ...
      console.error("Error updating child dental chart:", error);
      setFormMessage(`Error updating child dental chart: ${error instanceof Error ? error.message : 'Unknown error'}`);
      setFormType("error");
    }
  };
    ...
    return (
      <div className="bg-white p-6 rounded-lg shadow-md border-2 border-gray-300">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Child Surface Dental Chart - Edit Mode</h1>
        </div>
  ...
  const renderRootCanalChart = () => (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Child Root Canal Chart - Edit Mode</h1>
      </div>
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-lg">Loading child dental chart...</div>
      </div>
    );
  }
