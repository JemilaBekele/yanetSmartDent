...
const PatientComponent: React.FC<PatientDetailsProps> = ({ params }) => {
  ...
  const getRoleLinks = () => {
    const commonLinks = [
      { label: "Card", href: `/${role}/card/all/${patientId}` },
      { label: "Dental Record", href: `/${role}/medicaldata/medicalhistory/all/${patientId}` },
      { label: "Consent", href: `/${role}/Consent/all/${patientId}` },
      { label: "Appointments", href: `/${role}/appointment/all/${patientId}` },
      { label: "Note", href: `/${role}/Note/all/${patientId}` },
      { label: "Invoice", href: `/${role}/allinvoice/all/${patientId}` },
      { label: "Images", href: `/${role}/image/all/${patientId}` },
      { label: "Organization", href: `/${role}/organization/all/${patientId}` },
      { label: "Credit", href: `/${role}/creadit/all/${patientId}` },
      { label: "Prescriptions", href: `/${role}/prescriptions/all/${patientId}` },
      { label: "Proforma", href: `/${role}/Performa/all/${patientId}` },
      { label: "Medical Certificate", href: `/${role}/medicalcertificate/all/${patientId}` },
      { label: "Orthodontic Record", href: `/${role}/Orthodontics/all/${patientId}` },
      { label: "Referral", href: `/${role}/referral/all/${patientId}` },
      { label: "FNA", href: `/${role}/FNA/all/${patientId}` },
    ];
    ...
    const roleSpecificLinks = {
      ...
      nurse: [
        { label: "Health Information", href: `/nurse/medicaldata/healthinfo/all/${patientId}` },
      ],
      labratory: [
        { label: "Labratory", href: `/labratory/labratory/all/${patientId}` },
        { label: "Health Information", href: `/labratory/medicaldata/healthinfo/all/${patientId}` },
      ],
      reception: []
    };

    return [...commonLinks, ...(roleSpecificLinks[role as keyof typeof roleSpecificLinks] || [])];
  };
                ...
                <div className="grid gap-2">
                  {links.map(({ label, href }) => (
                    <Link key={label} href={href}>
                      <Button variant="outline" className="w-full justify-start h-9">{label}</Button>
                    </Link>
                  ))}
                </div>
              </ScrollArea>
                ...
                <div className="grid gap-2">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent className="w-64">
                      {[
                        { label: "Dental Records", href: `/${role}/medicaldata/medicalhistory/all/${patientId}` },
                        { label: "Orthodontics", href: `/${role}/Orthodontics/all/${patientId}` },
                        { label: "Prescriptions", href: `/${role}/prescriptions/all/${patientId}` },
                        { label: "Consent", href: `/${role}/Consent/all/${patientId}` },
                        { label: "Medical Certificate", href: `/${role}/medicalcertificate/all/${patientId}` },
                        { label: "Referral", href: `/${role}/referral/all/${patientId}` },
