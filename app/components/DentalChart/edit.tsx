...
type DentalChartEditProps = {
  params: {
    patientId: string; // Patient ID
    dentalChartId: string; // Dental Chart ID
  };
};
...
// Geometric Diagram Component for Surface Chart - FIXED
...
// Surface Chart Tooth Component - UPDATED with toggle functionality
const SurfaceTooth = ({ 
  toothNumber, 
  isSelected, 
  onClick, 
          ...
          <div className="absolute inset-0 z-10">
            ...
            <div
              ...
              onClick={(e) => {
                ...
                const newCondition = currentCondition ? null : "CARIES"; // Default condition
                handleSurfaceClick(toothNumber, "Buccal", newCondition || "CARIES");
              }}
              title="Buccal"
            ></div>
            ...
            <div
              ...
              onClick={(e) => {
                ...
                const newCondition = currentCondition ? null : "CARIES";
                handleSurfaceClick(toothNumber, "Lingual", newCondition || "CARIES");
              }}
              title="Lingual"
            ></div>
            ...
            <div
              ...
              onClick={(e) => {
                ...
                const newCondition = currentCondition ? null : "CARIES";
                handleSurfaceClick(toothNumber, "Mesial", newCondition || "CARIES");
              }}
              title="Mesial"
            ></div>
            ...
            <div
              ...
              onClick={(e) => {
                ...
                const newCondition = currentCondition ? null : "CARIES";
                handleSurfaceClick(toothNumber, "Distal", newCondition || "CARIES");
              }}
              title="Distal"
            ></div>
            ...
            {!isFourSurfaceTooth && (
              <div
                ...
                onClick={(e) => {
                  ...
                  const newCondition = currentCondition ? null : "CARIES";
                  handleSurfaceClick(toothNumber, "Occlusal", newCondition || "CARIES");
                }}
                title="Occlusal"
              ></div>
            )}
...
// Root Canal Tooth Component - UPDATED with toggle functionality
...
// Main Edit Component
export default function DentalChartEdit({ params }: DentalChartEditProps) {
  const { patientId, dentalChartId } = params;
  ...
  // Load existing dental chart by ID
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.data) {
            const dentalChart = data.data.DentalChart;
            
            setTeethData(dentalChart.teeth || []);
            setActiveTab(dentalChart.activeTab || "surface");
            setSelectedCondition(dentalChart.selectedCondition || "CARIES");
            setSelectedRootCanalCondition(dentalChart.selectedRootCanalCondition || "NORMAL");
            setBrushSize(dentalChart.brushSettings?.brushSize || 8);

            if (dentalChart.customRootLayers) {
              const transformedLayers: CustomRootLayer[] = dentalChart.customRootLayers.map((layer: any) => ({
          ...
          setFormMessage("Failed to load dental chart");
          setFormType("error");
        }
      } catch (error) {
        console.error("Error loading dental chart:", error);
        setFormMessage("Error loading dental chart");
        setFormType("error");
      } finally {
        setLoading(false);
      }
...
// Add this handler to update notes immediately when typing - REPLACE THE CURRENT ONE
const handleGeneralNoteChange = (newNote: string) => {
  ...
  const handleSurfaceToothClick = (toothNumber: number) => {
    ...
    if (newSelectedTooth) {
      ...
      setGeneralNote(existingTooth?.generalNote || existingTooth?.notes || "");
      
      // Add note for tooth selection
    } else {
      setGeneralNote("");
    }
  };
  ...
  const handleSurfaceClick = (toothNumber: number, surfaceName: string, condition: string) => {
      ...
      if (tooth.toothNumber === toothNumber) {
        ...
        const updatedSurfaces = tooth.surfaces?.map((surface: any) => {
          if (surface.name === surfaceName) {
            isNewSurface = false;
            if (surface.condition === condition) {
              return null; // Remove condition if same
            }
            return { ...surface, condition: condition };
          }
          return surface;
        }).filter(Boolean) || [];

        if (isNewSurface && condition) {
          updatedSurfaces.push({ name: surfaceName, condition: condition });
        }
    ...
    if (isNewTooth && condition) {
      updatedTeeth.push({
        toothNumber: toothNumber,
        surfaces: [{ name: surfaceName, condition: condition }],
        roots: [],
        notes: "",
        generalNote: "",
        overallStatus: "NORMAL"
    ...
    // Add note for surface interaction
    const action = condition ? `Applied ${condition} to ${surfaceName}` : `Cleared ${surfaceName}`;
  };
  ...
  const handleRootCanalToothClick = (toothNumber: number) => {
    ...
    if (newSelectedTooth) {
      const existingTooth = teethData.find(t => t.toothNumber === newSelectedTooth);
      setGeneralNote(existingTooth?.notes || existingTooth?.generalNote || "");
  ...
  // Handle save - Use PATCH to update existing dental chart
  // SIMPLIFIED Handle save function - notes are already in teethData
const handleSave = async () => {
  try {
    if (selectedTooth) {
      // Final sync - notes should already be in teethData from handleGeneralNoteChange
      const updatedTeeth = teethData.map(tooth => 
        tooth.toothNumber === selectedTooth 
          ? { 
              ...tooth, 
    ...
    setFormMessage("Updating dental chart...");
    setFormType(null);

    // Extract notes from teethData for the API
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || "Failed to update dental chart");
    }
    ...
    setFormMessage(`Error updating dental chart: ${error instanceof Error ? error.message : 'Unknown error'}`);
    setFormType("error");
  }
};
  ...
  const debugDataStructure = () => {
    ...
    console.log("Current Teeth Data:", teethData);
  };

  // Render Surface Chart - UPDATED with filtered conditions
    ...
    return (
      <div className="bg-white p-6 rounded-lg shadow-md border-2 border-gray-300">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Surface Dental Chart - Edit Mode</h1>
        </div>
  ...
  // Render Root Canal Chart
  const renderRootCanalChart = () => (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Root Canal Chart - Edit Mode</h1>
        <button
