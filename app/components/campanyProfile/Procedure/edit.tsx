...
export default function EditProcedureForm() {
  ...
  // Adjust textarea height when description changes or after data load
  useEffect(() => {
    adjustTextareaHeight();
  }, [formData.description]);

  // Fetch existing procedure data
  useEffect(() => {
    if (!id) return;
    ...
    const fetchProcedure = async () => {
      try {
        ...
        if (!response.ok) {
          setFormMessage(result.message || "Failed to load procedure data.");
          setFormType("error");
          return;
        }
        ...
        setFormMessage("An unexpected error occurred while loading data.");
        setFormType("error");
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setFormMessage(result.message || "Failed to update procedure.");
        setFormType("error");
        return;
      }

      setFormMessage("Procedure updated successfully!");
      setFormType("success");
      ...
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            Edit Procedure and Protocols
          </h1>
          <p className="text-gray-600">
            Update the procedure title and description.
          </p>
        </div>
