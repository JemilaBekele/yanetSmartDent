...
const WithdrawalDetailPage: React.FC<WithdrawalViewProps> = ({ id }) => {
  ...
  useEffect(() => {
    const fetchWithdrawal = async () => {
      try {
        if (id) {
          const response = await fetch(`/api/inventory/WithdrawalRequest/${id}`);
          if (!response.ok) {
  ...
  const handleStatusUpdate = async () => {
    ...
    setUpdating(true);
    try {
      const response = await fetch(`/api/inventory/WithdrawalRequest/approve/${id}`, {
        method: 'PATCH',
        headers: {
      ...
      setWithdrawal(prev => prev ? {
        ...
        ...(selectedStatus === WithdrawalStatus.ISSUED && { issuedAt: new Date().toISOString() })
      } : null);
  ...
  const isImmutable = [WithdrawalStatus.ISSUED, WithdrawalStatus.RETURNED].includes(withdrawal.status as WithdrawalStatus);
  ...
  const getStatusVariant = (status: string) => {
    switch (status) {
      ...
      case WithdrawalStatus.REJECTED:
        return 'destructive';
      case WithdrawalStatus.ISSUED:
        return 'default';
      case WithdrawalStatus.RETURNED:
        return 'secondary';
      default:
        return 'secondary';
    }
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 space-y-6">
          ...
          <CardContent className="bg-green-25">
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
              <Select
                ...
                onValueChange={(value: WithdrawalStatus) => setSelectedStatus(value)}
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value={WithdrawalStatus.ISSUED} className="text-green-600 focus:text-green-700">
                    Issued
                  </SelectItem>
                </SelectContent>
              </Select>
        ...
        <CardHeader className="bg-green-50 rounded-t-lg">
          <CardTitle className="text-2xl font-bold flex items-center gap-2 text-green-800">
            ...
            >
              ...
              ) : withdrawal.status === WithdrawalStatus.ISSUED ? (
                <>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <p>
                    <span className="font-medium text-green-800">Requested By:</span> {withdrawal.userId?.name || 'Unknown User'}
                  </p>
                </div>
                ...
                {withdrawal.createdBy && (
                  <div className="flex items-center gap-2">
                    <User className="h-4 w-4 text-green-600" />
                    <p>
                      <span className="font-medium text-green-800">Created By:</span> {withdrawal.createdBy.name}
                    </p>
                  </div>
                )}
                {withdrawal.updatedBy && (
                  <div className="flex items-center gap-2">
                    <User className="h-4 w-4 text-green-600" />
                    <p>
                      <span className="font-medium text-green-800">Updated By:</span> {withdrawal.updatedBy.name}
                    </p>
                  </div>
                )}
                {withdrawal.notes && (
              ...
              <Table>
                ...
                <TableBody>
                  {withdrawal.items.map((item, index) => (
                    <TableRow key={item._id || index} className="border-green-100 hover:bg-green-50">
                      ...
                      <TableCell>
                        {item.productUnitId?.unitOfMeasureId?.name || item.productUnitId?.name || 'N/A'}
                      ...
                      <TableCell className="font-medium text-green-700">{item.requestedQuantity}</TableCell>
      ...
      <div className="flex gap-4 justify-end">
        <Button 
          variant="outline" 
          onClick={() => router.push('/inventory/Withdrawal')}
        ...
        {withdrawal.status === WithdrawalStatus.PENDING && (
          <Button 
            onClick={() => router.push(`/inventory/Withdrawal/${withdrawal._id}`)}
