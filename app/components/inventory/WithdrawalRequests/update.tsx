...
export default function UpdateWithdrawalRequestPage({ params }: PageProps) {
  const { id } = params;
  ...
  const [existingRequest, setExistingRequest] = useState<WithdrawalRequestData | null>(null);
    ...
    try {
      ...
      reset({
        ...
        items: data.items.map((item: any) => ({
          ...
          availableQuantity: 0, // Will be updated when batch is selected
          personalStockId: item.personalStockId?._id || item.personalStockId,
        }))
      });
    } catch (error) {
  ...
  const onSubmit = async (data: WithdrawalRequestFormValues) => {
    try {
      ...
      for (const item of data.items) {
        ...
        if (stockItem) {
          // For this validation, we need to fetch the unit conversion factor
      ...
      if (!response.ok) throw new Error(result.message || 'Failed to update withdrawal request');
...
interface ItemRowProps {
  ...
  watch: any;
  isEditMode: boolean;
  existingRequest: WithdrawalRequestData | null;
}

function WithdrawalRequestItemRow({ 
  index, 
  control, 
  register, 
  personalStock, 
  ...
  // Convert available quantity to selected unit and check if any can be requested
  const availableQuantityInSelectedUnit = availableBaseQuantity / selectedUnitConversion;
  const canRequestAny = availableQuantityInSelectedUnit >= 1; // At least 1 full unit available
  ...
  // Check if the request is in a status that allows editing
  const isEditable = !isEditMode || (existingRequest && 
    ['pending', 'draft'].includes(existingRequest.status.toLowerCase()));
