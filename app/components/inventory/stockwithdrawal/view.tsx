...
const StockWithdrawalDetailPage: React.FC<StockWithdrawalViewProps> = ({ id }) => {
  ...
  const handleStatusUpdate = async () => {
    ...
    setUpdating(true);
    try {
      const response = await fetch(`/api/inventory/stockwithdrawal/approve/${id}`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    ...
    <TableHead className="text-green-800">To Location</TableHead>
      ...
      <div className="flex gap-4 justify-end">
        <Button 
          variant="outline" 
          onClick={() => router.push('/inventory/stockwithdrawal')}
        ...
        {withdrawal.status === StockWithdrawalStatus.PENDING && (
          <Button 
            onClick={() => router.push(`/inventory/stockwithdrawal/${withdrawal._id}`)}
...
export default StockWithdrawalDetailPage;