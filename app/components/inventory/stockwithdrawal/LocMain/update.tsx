...
export default function UpdateStockLocationMainWithdrawalPage({ params }: PageProps) {
  const { id } = params;
  ...
  const fetchStockWithdrawalRequest = async () => {
    try {
      const response = await fetch(`/api/inventory/loctoMainstockwithdrawal/${id}`);
  ...
  return (
    <div className="container mx-auto py-6">
      <h1 className="text-3xl font-bold mb-6">
        Edit Stock Withdrawal Request to Main Stock
      </h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button type="submit" disabled={submitting}>
            {submitting ? 'Updating Withdrawal Request...' : 'Update Withdrawal Request to Main Stock'}
          </Button>
        </div>
      </form>
    </div>
