...
export default function CreateStockLocationMainRequestPage() {
  ...
  const onSubmit = async (data: WithdrawalRequestFormValues) => {
    try {
      ...
      const response = await fetch('/api/inventory/loctoMainstockwithdrawal', {
        method: 'POST',
      ...
      router.push('/inventory/stockwithdrawal/LocMain');
    } catch (error: any) {
  ...
  return (
    <div className="container mx-auto py-6">
      <h1 className="text-3xl font-bold mb-6">Create Withdrawal Request to Main Stock</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea 
                id="notes" 
                {...register('notes')} 
                placeholder="Reason for withdrawal to main stock (optional)" 
              />
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button type="submit" disabled={submitting}>
            {submitting ? 'Creating Withdrawal Request...' : 'Create Withdrawal Request to Main Stock'}
          </Button>
        </div>
      </form>
    </div>
