...
const EditOrthodonticsForm = ({ params }: OrthoFindingFormProps) => {
 ...
 const [formData, setFormData] = useState<FormData>({
  ...
  const handleInputChange = (
   ...
   useEffect(() => {
      async function fetchFinding() {
        try {
          const response = await fetch(`/api/Orthodontics/all/${findingId}`);
          if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.patch(`/api/Orthodontics/all/${findingId}`, formData);
      ...
      <div className="flex-grow md:ml-60 container mx-auto p-4">
        <div className="flex space-x-8">
          ...
          <div className="w-2/3 bg-white p-6 rounded-lg shadow-md">
            <h1 className="text-2xl font-bold mb-4">Edit Orthodontic Treatment Form</h1>
             <Image
  ...
  {/* Render checkboxes except 'other' */}
  {Object.keys(formData.ChiefComplaint).map((key) => (
    key !== "other" ? (
      ...
      />
    ) : null // Don't render 'other' here
  ))}

  {/* Render 'Other' input at the end */}
  <InputField
    key="other"
    label="Other"
  ...
  {formData.TreatmentPlan.map((plan, index) => (
    <div key={index} className="border p-4 mb-4">
      {Object.entries(plan).map(([key, value]) => {
        // Skip the "_id" key and handle the remaining keys
      ...
      <button
        ...
        className="mt-2 p-2  text-red-500rounded"
      >
         <DeleteOutlined/>
      </button>
    </div>
...
export default EditOrthodonticsForm;
