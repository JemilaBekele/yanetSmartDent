...
export default function CreditForm({ params }: InvoiceFormProps) {
  ...
  const [selectedDoctor, setSelectedDoctor] = useState<{ id: string; username: string } | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/Creadit/recep/${patientId}`, {
        method: "POST",
      ...
      if (response.ok) {
        ...
        router.push(
        `/reception/creadit/all/${patientId}` 
        );
      } else {
  ...
  const columns: Column<CreditItem>[] = [
    ...
    {
      header: "Amount",
      key: "amount" as keyof CreditItem,
          ...
          <div className="w-auto bg-white p-6 rounded-lg shadow-md">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex space-x-6 mb-4">
                ...
                <div className="w-1/3">
                  <label className="font-bold">Select Doctor:</label>
                  <select
