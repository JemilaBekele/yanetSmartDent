...
export default function PerInvoiceForm({ params }: InvoiceFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
     ...
     if (response.ok) {
        ...
        router.push(role === "admin" ? `/admin/Performa/all/${patientId}` : `/doctor/Performa/all/${patientId}`);
      } else {
        ...
        <div className="flex space-x-4">
          ...
          <div className="w-auto bg-white p-6 rounded-lg shadow-md">
            <header className="text-center mb-6">
              <h1 className="text-2xl font-bold">Add Proforma Record</h1>
