...
export async function GET(request: NextRequest, ) {
    ...
    try {
      ...
      const invoice = await Perfo.findById({ _id: invoiceId });
  ...
  export async function DELETE(request: NextRequest) {
    ...
    try {
      ...
      const deletedInvoice = await Perfo.findByIdAndDelete(invoiceId).exec();
      if (!deletedInvoice) {
        return NextResponse.json({ error: "Invoice not found" }, { status: 404 });
      }
      ...
      const patient = await Patient.findOneAndUpdate(
        { Perfo: invoiceId }, // Find patient with this invoice ID
        { $pull: { Perfo: invoiceId } }, // Remove the invoice ID from the array
 ...
 export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const invoice = await Perfo.findById(InvoiceId).exec();
    if (!invoice) {
      return NextResponse.json({ error: "Invoice not found" }, { status: 404 });
    }
